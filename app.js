...
server.get('/flights/from/:origen/to/:destino/date_1/:ida/adults/:adultos/date_2/:vuelta/kids/:ninios/age/:edad', (req, res) => {
    ...
    let scrapearDatosCzech = async() => {
        ...
        try {
            ...
            const browser = await puppeteer.launch();
            const page = await browser.newPage();
            await page.goto("https://www.eurowings.com/es/reservar/vuelos/busqueda-de-vuelos.html?destination=LHR&triptype=r&origin=LPA&fromdate=2020-10-25&todate=2020-11-14&adults=1&childs=0&infants=0&lng=es-ES#/reservar-vuelos/select");
            setTimeout(async() => {
                let body = await page.content()
                const $ = cheerio.load(body)

                console.log($('.m-ibe-flighttable__item .m-ibe-flighttable__item-cell.m-ibe-flighttable__flight .m-ibe-flighttable__station').text().first())
                    /*ret.datosIda = {
                        title: 'Vuelo de ida',
                        empresa: $('.airline-name').first().text(),
                        origin: {
                            aeropuertoSalida: $('.m-ibe-flighttable__station').html(),
                            origen: ,
                        ...
                        destiny: {
                            aeropuertoLlegada: $('.flight-details-arrival > .flight-details-airport').first().text(),
                            destino: $('.flight-details-arrival > .flight-details-city').first().text().split(' ')[1],
                            horarioLlegada: $('.m-ibe-flighttable__item-cell.m-ibe-flighttable__flight.m-ibe-flighttable__flight--right .a-headline.a-headline--h4.t-spacing--0').html().split('<')[0]
                        },
                        price: $('.availability-group-cell-price > .cell-reco-currency').first().text() + ' ' + $('.cell-reco-bestprice-integer.availability-font-xlarge').first().text(),
                        duration: $('.duration').children().text().split(',')[0]
                    }
                    console.log(datosIda)
                    ret.datosVuelta = {
                        title: 'Vuelo de Vuelta',
                        empresa: $('.airline-name').first().text(),
                        origin: {
